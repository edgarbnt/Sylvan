...
| critic | Can a critic learned from lived episodes replace the hand-coded cost? | **Not yet, and the reason is now precise.** The critic was only ever scoring ~3 % of replans in the sparse world (it was gated on *both* resources being visible at once); the hand-coded cost silently drove the other 97 %. Let it decide 100 % and foraging halves (41 → 20 consumptions), *despite* four principled, separately verified fixes: a corpus matched to deployment, mirror symmetry enforced (the value was rating mirror-identical states 0.13 apart — pure learned noise), exploration added to collection, and the resulting off-axis value peak resolved. Every correct fix made it forage *worse* — which says the fault is not the data |
...
- **Mode 2 (deliberation)**: the planner imagines `(v, ω)` rollouts through the world-model and scores them. Perception is clean — both drives are read from the world-model's own object-centric slots, symmetrically, and a resource out of view is simply unknown (no oracle, no stale position). **Scoring is not**: a **critic learned from lived experience** (LeCun's trainable critic) is implemented, offline-sound (held-out AUC 0.99) and available (`SYLVAN_CRITIC_ALWAYS=1`), but when it actually drives every decision it forages roughly half as well as the hand-coded cost — so the live default still leans on that hand-coded cost. The gap is diagnosed, not hand-waved: the dream horizon spans ~0.8 m while resources sit 2 to 8 m out, so the 33 candidate plans are near-tied (best-vs-second margin ~0.004) and the planner asks the critic to *rank* near-identical options — a task it was never trained for (it was fit to *predict* Monte-Carlo returns). The hand-coded cost sidesteps this with a heading term whose directional pull accumulates across replans.
- **Current frontier**: **making the learned critic actually competent enough to take the wheel.** The next probe is a sharp one: train it directly on the *analytic* cost's own value outputs. If it then matches that cost, the architecture is sound and only the learning signal was wrong (Monte-Carlo returns teach prediction, not ranking); if it still fails while copying the right answer, the fault is the critic's architecture or how the planner consumes it. Either outcome is decisive. Behind that: **continual learning** (the day/night cycle) — though a naive version is now *known* to be futile here, since retraining on self-generated data provably re-confirms the agent's own biases. The slot's position readout is still a hand-designed geometry (a learned readout destabilised it). The sparse-world survival ceiling is **not** a decision problem: a free metabolic probe showed the world is *marginal* — sustainable foraging reaches 4.0 m while resources spawn 2 to 8 m out, so about two thirds of lives are lost to arithmetic whatever the agent decides. See `docs/audit_lecun_2026-07-06.md` for the full purity audit.

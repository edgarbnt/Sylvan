...
- **Current frontier**: **perception grounded in lived consequence.** The stated end goal is a world that looks like the real one, where appearances change, and this frontier is now essentially closed. Danger is learned from what preceded damage (a saliency lens over the raw retina, judged at parity with the hand rule and promoted). The resource queries are measured from the rendered world and their meaning discovered by contingency, not written. And a credit-assignment chantier proved the entity can attribute a consequence to the right cue among several co-occurring appearances, and recognise a *neutral* object that means nothing (the berry-in-bush case, gated G0 through G3, offline to closed-loop). The last hand-written appearance key is dissolved: nothing in the decision loop presumes the world's colors. The learned-query world-model that carries the neutral class is kept as a validated artifact rather than the default, because the crowded three-color space forces a margin trade-off (a neutral distractor sitting close to water in cosine-RGB cannot be excluded without also clipping some real water, a measured limit of a color-only retina). Next, in order: the **obstacle / affordance channel** (a new *nature* of consequence, learned from world-model prediction error, which also builds the topology that makes navigation, search and memory matter), then **search and spatial memory** (the agent is still blind-by-wandering when nothing is visible, the biggest remaining capability hole). The sparse-world survival ceiling is **not** a decision problem: a free metabolic probe showed sustainable foraging reaches 4.0 m while resources spawn 2 to 8 m out, so about two thirds of lives are lost to arithmetic whatever the agent decides. See `docs/design_attribution_credit.md` for the credit chantier and `docs/design_purete_hjepa.md` for the purity map.

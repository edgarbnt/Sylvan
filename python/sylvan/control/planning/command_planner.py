...
class CommandPlanner:
    ...
    def plan(
        ...
        _slots0 = None
        _vis = None
        _ret0 = None   # rétine t0 (144) — visibilité/positions des slots ; None si pas multi-slot
        ...
        if surv_mode:
            ...
            bear_w = torch.atan2((wx - x) * torch.cos(yaw) - (wz - z) * torch.sin(yaw),
                                 (wx - x) * torch.sin(yaw) + (wz - z) * torch.cos(yaw))
            rate = max(cfg.surv_turn_rate, 1e-6)
            # ── JUGES CODÉS-MAIN DU DANGER : PURGÉS (hygiène 2026-07-16) ─────────────────────────
            # Détour gradué + centre corrigé (SYLVAN_HAZARD_DETOUR/CENTER_SHIFT/AVOID_RADIUS),
            # mur-vert (GREENWALL) et répulsion (AVOID/AVOID_REACH) : tous ÉLIMINÉS par le verdict
            # du 2026-07-16 (aucun ne casse le troc évitement↔repas dans la fente MPC — 33 arcs
            # myopes ne composent pas un contournement, quel que soit le score ; commit 8a3d80a).
            # Le contournement vit à l'ÉTAGE WAYPOINT ; l'arbitrage du sprint y est APPRIS
            # (critique composé, juge PASS 45/8 — docs/design_critique_sprint.md).

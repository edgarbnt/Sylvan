...
tick, segmente en POURSUITES (conventions v3 partagées avec les trainers), classe chaque décision :
...
dégâts payés (creux de santé), mort, pas restants. Le loader vit dans
scripts/train_sprint_critic.py (parité diag = trainer).
...
from scripts.train_sprint_critic import (_CFG, _INTR_EPS, load_corpus, load_sprint_decisions,
                                         measured_drain)
...
def report(rows: list[dict], drain: float) -> None:
    ...
    blesse = "NA (collecte ε requise)" if ncb < 20 or nrb < 20 else ("OK" if ucb < urb else "CONTREDIT")
...
def main() -> None:
    ...
    if not rows:
        ...
        return
    report(rows, measured_drain(args.runs))

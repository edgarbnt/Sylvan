...
## ⭐⭐⭐ VERDICT DU JUGE (2026-07-16) : **PASS** — L'ORACLE MEURT, LE SPRINT EST APPRIS
2×24 vies, seeds 1+2, monde v2, bras `SYLVAN_WP_SPRINT_CRITIC` (composed_v1) :
| bras | repas | morts-danger | réf géométrie | plafond oracle |
|---|---|---|---|---|
| s1 | 19 | 5 | 19/6 | 23/3 |
| s2 | **26** | **3** | 15/5 | 24/6 |
| **poolé** | **45 ✓ (≥42)** | **8 ✓ (≤13)** | 34/11 | 47/9 |
+11 repas poolés (> bruit ±5) ET −3 morts-danger vs géométrie ; au seed dur (s2) le composé BAT
l'oracle (26/3 vs 24/6) ; poolé = niveau plafond (45/8 vs 47/9, dans le bruit) avec moins de morts.
**CONSÉQUENCES (per pré-enregistrement)** : la règle-oracle `SYLVAN_WP_ORACLE_SPRINT` est RETIRÉE
du chemin vivant (sonde historique superseded, plus jamais un bras de référence) ; l'arbitrage du
sprint est désormais APPRIS — première décision apprise validée à l'étage waypoint. Config vivante
recommandée : `SYLVAN_WAYPOINT=1 SYLVAN_WP_SPRINT_CRITIC=data/checkpoints/sprint_critic/sprint_best.pt`.
Leçon de doctrine (3 chantiers, 5 A/B) : liens APPRIS (P̂ consommation, douleur̂) × valuation du
CORPS (constantes mesurées) + socle analytique consistant + remise plafonnée = la forme qui marche ;
ni le remplacement, ni le monolithe sur label net. Caveats honnêtes : gates offline 2/4 (G-rank/G-res
= plafond de bruit du label, jugés par l'owner) ; s1 = parité repas (le gain vit sur s2, comme
l'oracle) ; multi-seed au-delà de 1+2 = dette notée (comme s2/kinématique).

...
if [ "${RECOLLECT:-0}" != "1" ] && [ "$NA" -ge 60 ] && [ "$NB" -ge 60 ]; then
  ...
  bash scripts/collect_wm_eatrich.sh 60 41 14 0.10 100 retina_eat_a 0.65 2>&1 | grep -E "épisodes=|repas\(ate\)|repas BASSE|énergie GLOB"
  echo "### run B : 60 ép, seed 77, gate 0.82 (plus d'événements) ###"
  bash scripts/collect_wm_eatrich.sh 60 77 14 0.10 100 retina_eat_b 0.82 2>&1 | grep -E "épisodes=|repas\(ate\)|repas BASSE|énergie GLOB"
fi

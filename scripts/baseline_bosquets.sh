...
SPACING=${SPACING:-9.0}        # traversée = 818 ticks = 41 pts d'énergie (payable plein, mortelle à moitié)
ROOT=/home/edgarbrunet/Documents/PERSO/SylvanV1; cd "$ROOT" || exit 1
WM=${WM_CKPT:-data/checkpoints/wm_objcentric_kin/wm_best.pt}
TAG="bosq_r${REGROW}_b${BERRIES}_s${SEED}"

echo "=== BOSQUETS : regrow=$REGROW berries=$BERRIES/ressource patches=$PATCHES espacement=$SPACING ==="
...
SYLVAN_FOOD_COUNT=$BERRIES SYLVAN_WATER_COUNT=$BERRIES \
SYLVAN_FOOD_PATCHES=$PATCHES SYLVAN_WATER_PATCHES=$PATCHES \
SYLVAN_FOOD_PATCH_SPACING=$SPACING SYLVAN_WATER_PATCH_SPACING=$SPACING \
SYLVAN_FOOD_REGROW=$REGROW SYLVAN_WATER_REGROW=$REGROW \
SYLVAN_FOOD_MIN_RADIUS=3.0 SYLVAN_WATER_MIN_RADIUS=3.0 \
SYLVAN_FOOD_SPAWN_RADIUS=11.0 SYLVAN_WATER_SPAWN_RADIUS=11.0 \

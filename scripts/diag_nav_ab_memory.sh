...
#
# Usage: bash scripts/diag_nav_ab_memory.sh [dist=3.0] [eps_per_angle=2] [max_steps=700]
#          MEM=on  bash scripts/diag_nav_ab_memory.sh   → mémoire ON,  cone 180°
#          MEM=off bash scripts/diag_nav_ab_memory.sh   → mémoire OFF, cone 180°
#          MEM=on  SYLVAN_OCCLUDE_FOV_DEG=360 bash scripts/diag_nav_ab_memory.sh  → mémoire ON, 360°
set +e

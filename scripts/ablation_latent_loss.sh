...
# Puis diagnostics/diag_dream_disp.py sur chacun (path du rêve droit 120 pas + cohérence latente + eff_rank).
#
...
#     food-aware passe en mse (ou hybride mse+cosine). Re-passer diagnostics/diag_cem.py comme gate ENSUITE.
...
echo "=== abl_mse ==="
SYLVAN_WM_USE_RETINA=1 ./env_pytorch_3.12/bin/python diagnostics/diag_dream_disp.py data/checkpoints/abl_mse/wm_best.pt 1 2>&1 | grep -v -i warning
echo "=== abl_cosine ==="
SYLVAN_WM_USE_RETINA=1 ./env_pytorch_3.12/bin/python diagnostics/diag_dream_disp.py data/checkpoints/abl_cosine/wm_best.pt 1 2>&1 | grep -v -i warning
